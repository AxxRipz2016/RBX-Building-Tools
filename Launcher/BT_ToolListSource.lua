--[[ Встроенный UI/Dock/ToolList.lua для remote (игра не читает ваш локальный файл) ]]
return [[
...
function ToolList:render()
	...
	if toolCount > 0 and not self.__bt_logged then
		self.__bt_logged = true
		warn("[BT] ToolList:render, кнопок:", toolCount)
	end
	...
	return new('Frame', {
		...
		Size = UDim2.fromOffset(70, math.max(245, 35 * math.ceil(math.max(1, toolCount) / 2)));
	}, frameChildren)
end
